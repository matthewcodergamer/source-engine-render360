...
// Phase 3/4 keeps retail VPK payloads browser-backed in WORKERFS, but Source's
...
// names from the shared FS; filesystem_stdio intercepts actual opens/stats and
// reads real File/Blob ranges on the Source pthread. No VPK payload, maps,
// textures or audio are copied into MEMFS.
;(() => {
	'use strict'
	...
	function fail(error) {
		...
		const message = `[Render360 Phase 3/4] startup metadata staging failed: ${detail}`
		try { globalThis.render360SetPhase?.('phase3-startup-metadata-failed') } catch(_) {}
		Module.printErr?.(message)
		if(typeof render360Report === 'function') {
			try { render360Report('Phase 3/4 startup metadata failure', detail, error) } catch(_) {}
		}
		if(typeof abort === 'function') {
			abort(message)
			return
	...
	async function stage(files) {
		...
		Module.print?.(`[Render360 Phase 3/4] staged ${count} startup metadata files (${bytes} bytes) plus ${vpkPlaceholders} zero-byte VPK namespace placeholders; retail VPK payload remains browser-backed`)
...
if (typeof globalThis !== 'undefined' && globalThis.addEventListener) {
	globalThis.addEventListener('error', event => {
		...
		console.error('[Render360 worker/global error]', message, error && error.stack ? error.stack : error || '')
	})
	...
	globalThis.addEventListener('unhandledrejection', event => {
		...
		console.error('[Render360 worker/global unhandled rejection]', reason && reason.stack ? reason.stack : reason)
	})
}
